...
async function main(): Promise<void> {
  ...
  const stockAgent = await client.createAgent(modelDeploymentName, {
    ...
    instructions:
      "Your job is to get the stock price the requested company. If you don't know the realtime stock price, return the last known stock price.",
    onResponse: (response): void => {
      ...
      console.log(`STOCK-PRICE-AGENT: Received response with status: ${status}`);
    },
  });
    ...
    onResponse: (response): void => {
      const parsedBody =
        typeof response.parsedBody === "object" && response.parsedBody !== null
          ...
          : null;
      const status = parsedBody && "status" in parsedBody ? parsedBody.status : "unknown";
      console.log(`MY-AGENT: Received response with status: ${status}`);
    }
  });
    ...
    onResponse: (response): void => {
      const parsedBody =
        typeof response.parsedBody === "object" && response.parsedBody !== null
          ...
          : null;
      const status = parsedBody && "status" in parsedBody ? parsedBody.status : "unknown";
      console.log(`THREAD: Received response with status: ${status}`);
    }
  });
  ...
  // Create a message
  const message = await client.messages.create(thread.id, "user", "What is the stock price for Microsoft?", {
    onResponse: (response): void => {
      const parsedBody =
        typeof response.parsedBody === "object" && response.parsedBody !== null
          ? response.parsedBody
          : null;
      const status = parsedBody && "status" in parsedBody ? parsedBody.status : "unknown";
      console.log(`CREATE-MESSAGE: Received response with status: ${status}`);
    }
  });
  ...
  for (const m of messages) {
    if (m.role === "user") {
      console.log(`\n❓ USER QUESTION: ${Array.isArray(m.content) && m.content[0]?.type === "text" && 'text' in m.content[0]
